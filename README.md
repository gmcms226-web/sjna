...
이번 클론코딩을 통해 웹사이트 코드의 전반적인 구조와 흐름을 체감할 수 있었습니다.
단순히 코드를 따라 치는 것이 아니라, 실제 서비스되는 사이트를 분석하고 재현하면서 HTML/CSS/JS가 어떻게 유기적으로 맞물리는지 이해하게 되었습니다.
작업하면서 슬라이더 커스터마이징, CSS 파일 분리, 이미지 경로 오류, 반응형 레이아웃 깨짐, API 연동 등 다양한 문제를 직접 마주했습니다. 단순히 코드를 작성하는 것을 넘어 문제를 파악하고 원인을 찾아 해결하는 과정 자체가 큰 공부가 되었습니다. 특히 브라우저 개발자 도구를 적극 활용하며 오류를 추적하는 습관을 기를 수 있었습니다.
또한 공통 스타일을 common.css로 분리하고 페이지별로 역할을 나누는 경험을 통해, 단순히 동작하는 코드가 아닌 유지보수하기 좋은 코드 구조의 중요성을 느낄 수 있었습니다.
OpenWeatherMap API를 연동하여 실시간 날씨를 표시하는 기능을 직접 구현해보면서, API가 실제 서비스에서 어떻게 활용되는지도 경험할 수 있었습니다.
